					...
					bool has_lcdck)
{
	...
	struct clk_hw *parent_hws[2], *smdck_hw, *usbck_hw;
	...
	parent_data[0] = AT91_CLK_PD_HW(at91sam9x5_pmc->chws[PMC_MAIN]);
	hw = at91_clk_register_utmi(regmap, NULL, "utmick", NULL, parent_data);
	if (IS_ERR(hw))
		goto err_free;
