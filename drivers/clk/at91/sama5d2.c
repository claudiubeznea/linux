...
static void __init sama5d2_pmc_setup(struct device_node *np)
{
	...
	parent_data[0] = AT91_CLK_PD_HW(sama5d2_pmc->chws[PMC_MAIN]);
	hw = at91_clk_register_utmi(regmap, regmap_sfr, "utmick", NULL, parent_data);
	if (IS_ERR(hw))
		goto err_free;
	...
	hw = at91_clk_register_master_div(regmap, "masterck_div", NULL, parent_data,
					  &at91sam9x5_master_layout,
					  &mck_characteristics, &mck_lock,
					  CLK_SET_RATE_GATE, 0);
	if (IS_ERR(hw))
	...
	if (regmap_sfr) {
		...
		parent_hws[1] = sama5d2_pmc->ghws[54]; /* i2s0_gclk */
