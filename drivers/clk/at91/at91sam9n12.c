...
{
	struct clk_hw *parent_hws[2], *usbck_hw, *hw, *main_rc_hw, *main_osc_hw;
	...
	parent_data[0] = AT91_CLK_PD_HW(at91sam9n12_pmc->chws[PMC_PLLBCK]);
	usbck_hw = at91sam9n12_clk_register_usb(regmap, "usbck", NULL, parent_data);
	if (IS_ERR(usbck_hw))
		goto err_free;
